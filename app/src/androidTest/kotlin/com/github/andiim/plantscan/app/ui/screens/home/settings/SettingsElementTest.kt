...
class SettingsElementTest {
  ...
  @Mock private val accountService = mock(AccountService::class.java)
  @Mock private val logService = mock(LogService::class.java)
  ...
  fun should_appear_when_starting_as_anonymous() {
    `when`(accountService.currentUser).thenReturn(flow { emit(User("user", true)) })
  ...
  fun should_appear_when_starting_as_user() = runTest {
    `when`(accountService.currentUser).thenReturn(flow { emit(User("user", false)) })
    viewModel = SettingsViewModel(accountService, logService)
    composeRule.setContent {
      PlantScanTheme {
        Surface {
