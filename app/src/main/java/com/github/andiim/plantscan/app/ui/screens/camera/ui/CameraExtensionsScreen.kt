...
class CameraExtensionsScreen(
    ...
    private var showGallery: (() -> Unit)? = null
    ...
    init {
        ...
        with(binding) {
            ...
            val scope: LifecycleCoroutineScope? =
                rootView.findViewTreeLifecycleOwner()?.lifecycleScope
            ...
            switchLensButton.setOnClickListener { switchLens(rootView, it) }
            ...
            permissionsRationaleContainer.apply {
                setViewCompositionStrategy(ViewCompositionStrategy.DisposeOnDetachedFromWindow)
                setContent {
                    NonPermissionScreen(
                        shouldShowRationale = rationaleState.value,
                        showPermission = {
                            scope?.launch {
                                _action.emit(CameraUiAction.RequestPermissionClick)
                            }
                        },
                        openGallery = { showGallery?.invoke() }
                    )
                }
            }
    ...
    fun showPermissionsRequest(shouldShowRationale: Boolean, galleryClicked: () -> Unit) {
    ...
    private fun switchLens(rootView: View, switchLensButton: View) {
