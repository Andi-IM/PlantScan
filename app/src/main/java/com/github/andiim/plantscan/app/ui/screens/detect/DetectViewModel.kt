...
constructor(
    ...
    fun detect(context: Context) {
        ...
        launchCatching(logService) {
            ...
            val confidence = 40
            useCase.detect(base64Data, confidence).collectLatest {
                when (it) {
                    is Resource.Error -> {
                    ...
                    is Resource.Success -> {
                        ...
                        visualizedResult?.let {
                            data = data.apply { image = image.copy(data = it) }
                        }
    ...
    private fun Bitmap.convertToBase64(compressionLevel: Int = 20): String {
        val outputStream = ByteArrayOutputStream()
        this.compress(Bitmap.CompressFormat.JPEG, compressionLevel, outputStream)
